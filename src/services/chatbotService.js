...
const NO_SORRY_RULE    = 'CRITICAL: Never say "sorry", "I apologize", "I cannot", "I could not", "I am unable", or any refusal/apology phrase. Always provide an educational answer even if it must come from general knowledge.';
const NO_SORRY_RULE_AR = 'قاعدة صارمة: لا تقل أبداً "آسف" أو "لا أستطيع" أو "لا يمكنني الإجابة" أو أي عبارة اعتذار أو رفض. قدّم دائماً إجابة تعليمية مفيدة حتى لو كانت من المعرفة العامة.';
...
const polishAnswer = ({ rawAnswer, question, questionStyle, chunks }) => {
  const noApology = stripApologyOpening(rawAnswer);
  const noThink = stripThinkTags(noApology || rawAnswer);
...
const SORRY_PATTERN = /^[\s]*(?:sorry[,.]?\s*i\s+(?:could\s+not|couldn't|cannot|can't)\s+(?:get|find|provide|give)\s+(?:an?\s+)?answer[^.]*\.?\s*(?:please\s+try\s+again[^.]*\.?)?|i\s+(?:apologize|am\s+sorry)[^.]*\.|sorry[,.]?\s+(?:but\s+)?i\s+(?:don'?t|do\s+not)[^.]*\.|آسف[^.]*\.|عذراً[^.]*\.)/iu;

const stripApologyOpening = (text) => {
  if (!text) return text;
  // If the whole response is an apology/sorry, clear it so fallback triggers
  if (SORRY_PATTERN.test(text.trim())) return '';
  return text;
};
...
const polishGeneralAnswer = (rawAnswer) => {
  ...
  cleaned = stripApologyOpening(cleaned);
...
export const askChatbot = async ({ tokenUser, question, courseId, subjectId, lessonId, history = [] }) => {
  ...
  if (!selected) {
    ...
    // Zero relevance: nothing in the lesson/subject/course content matched the
    // question strongly enough. Per CLAUDE.md, do NOT reach for web search or
    // general LLM knowledge here — refuse immediately with the standard wording.
    return makeRefusal({
      scope,
      reason: lang === 'en'
        ? 'This topic is not covered in the lesson materials.'
        : 'هذا غير مغطى بشكل واضح في محتوى هذه الحصة.',
      mode: refusalMode,
      lang,
    });
  }
  ...
  if (confidence < MIN_CONFIDENCE_TO_ANSWER || metaQuestion) {
    // Low (but nonzero) relevance: the lesson touches the topic without fully
    // covering it. Prefer supplementing with a web search over letting the LLM
    // fill the gaps from its own training knowledge — falls back to the hybrid
    // lesson+general-knowledge blend below when web search is unavailable/empty
    // (e.g. no TAVILY_API_KEY configured) or for meta-questions about the lesson.
    if (!metaQuestion) {
      try {
        ...
        // web search failed — fall through to hybrid answer
      }
    }
