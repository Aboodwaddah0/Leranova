...
export const validateAddUserData = (data) => {
	...
	if (role === 'STUDENT' && !phone) {
		errors.push("Phone number is required for STUDENT");
	}

	const fatherPhone =
		data?.fatherPhone === undefined || data?.fatherPhone === null || String(data.fatherPhone).trim() === ""
			? null
			: String(data.fatherPhone).trim();
...
export const validateExcelData = (data, options = {}) => {
	...
	data.forEach((row, index) => {
		...
		const rawFirstName = getRowValue(row, "firstName") ?? getRowValue(row, "FirstName") ?? getRowValue(row, "first_name");
		const rawLastName  = getRowValue(row, "lastName")  ?? getRowValue(row, "LastName")  ?? getRowValue(row, "last_name");
		...
		const rawPhone =
			...
			getRowValue(row, "Phone") ??
			getRowValue(row, "PhoneNumber") ??
			getRowValue(row, "phone_number");
		const rawFatherPhone =
			getRowValue(row, "fatherPhone") ??
			getRowValue(row, "FatherPhone") ??
			getRowValue(row, "father_phone") ??
			getRowValue(row, "parentPhone");
		...
		const parentNationalId =
			rawParentNationalId === null || rawParentNationalId === undefined || String(rawParentNationalId).trim() === ""
				...
				: normalizeNationalId(rawParentNationalId);
		const phone =
			rawPhone === null || rawPhone === undefined || String(rawPhone).trim() === ""
				? null
				: String(rawPhone).trim();
		const fatherPhone =
			rawFatherPhone === null || rawFatherPhone === undefined || String(rawFatherPhone).trim() === ""
				? null
				: String(rawFatherPhone).trim();
		...
		if (role === "STUDENT") {
			...
			if (!phone) {
				errors.push(`Row ${rowNumber}: Phone is required for STUDENT rows`);
			}
		}
