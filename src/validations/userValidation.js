...
export const validateAddUserData = (data) => {
	...
	const fatherName =
		data?.fatherName === undefined || data?.fatherName === null || String(data.fatherName).trim() === ""
			? null
			: String(data.fatherName).trim();
...
export const validateExcelData = (data, options = {}) => {
	...
	data.forEach((row, index) => {
		...
		const rawFatherName =
			getRowValue(row, "fatherName") ??
			getRowValue(row, "FatherName") ??
			getRowValue(row, "father_name") ??
			getRowValue(row, "parentName");
		...
		const fatherName =
			rawFatherName === null || rawFatherName === undefined || String(rawFatherName).trim() === ""
				? null
				: String(rawFatherName).trim();
