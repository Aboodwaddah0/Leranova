...
import { fetchAnalytics } from '../services/instructorService';
import type { InstructorAnalytics } from '../../../types/instructor';
...
export function InstructorAnalyticsTab({ isSchool }: Props) {
  ...
  return (
    ...
    >
      ...
      {overview && (
        <View style={styles.statsGrid}>
          {[
            ...
            { label: 'Quiz Pass %',    value: `${overview.quizPassRate.toFixed(0)}%`, icon: CheckCircle, color: '#34d399', bg: 'rgba(52,211,153,0.12)' },
      ...
      {/* AI Coaching */}
      {aiCoaching && !aiCoaching.fallback && (
        <View style={[styles.card, { backgroundColor: 'rgba(99,102,241,0.06)', borderColor: 'rgba(99,102,241,0.2)' }]}>
          <View style={styles.cardTitleRow}>
            <Brain size={16} color="#6366f1" />
            <Text style={[styles.cardTitle, { color: '#6366f1' }]}>AI Coaching</Text>
          </View>
          {!!aiCoaching.summary && (
            <Text style={[styles.aiSummary, { color: T.subtext }]}>{aiCoaching.summary}</Text>
          )}
          {(aiCoaching.keyInsights ?? []).length > 0 && (
            <>
              <Text style={[styles.subSectionTitle, { color: T.text }]}>Key Insights</Text>
              {aiCoaching.keyInsights!.map((insight, i) => (
                <View key={i} style={styles.bulletRow}>
                  <Text style={{ color: '#6366f1', fontSize: 16 }}>•</Text>
                  <Text style={[styles.bulletText, { color: T.subtext }]}>{insight}</Text>
                </View>
              ))}
            </>
          )}
          {(aiCoaching.actionItems ?? []).length > 0 && (
            <>
              <Text style={[styles.subSectionTitle, { color: T.text }]}>Action Items</Text>
              {aiCoaching.actionItems!.map((item, i) => (
                <View key={i} style={styles.bulletRow}>
                  <Text style={{ color: '#10b981', fontSize: 16 }}>→</Text>
                  <Text style={[styles.bulletText, { color: T.subtext }]}>{item}</Text>
                </View>
              ))}
            </>
          )}
      ...
      {subjectPerformance.length > 0 && (
        <View style={[styles.card, { backgroundColor: T.surface, borderColor: T.border }]}>
          ...
          {subjectPerformance.map(s => (
            <View key={s.subjectId} style={styles.perfRow}>
              <Text style={[styles.perfName, { color: T.text }]} numberOfLines={1}>{s.subjectName}</Text>
      ...
      {atRiskStudents.length > 0 && (
        <View style={[styles.card, { backgroundColor: T.surface, borderColor: T.border }]}>
          ...
          {atRiskStudents.map(s => (
            <View key={s.studentId} style={[styles.atRiskRow, { borderBottomColor: T.border }]}>
              <View style={{ flex: 1 }}>
                <Text style={[styles.name, { color: T.text }]}>{s.name}</Text>
                <Text style={[styles.sub, { color: T.muted }]}>
                  {s.daysInactive}d inactive · {s.completionRate.toFixed(0)}% completion
                </Text>
              </View>
      ...
      {topStudents.length > 0 && (
        <View style={[styles.card, { backgroundColor: T.surface, borderColor: T.border }]}>
          ...
          {topStudents.slice(0, 5).map((s, i) => (
            <View key={s.studentId} style={[styles.topRow, { borderBottomColor: T.border }]}>
          ...
          {activityFeed.slice(0, 10).map((a, i) => (
            <View key={i} style={[styles.feedRow, { borderBottomColor: T.border }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.sub, { color: T.muted }]}>{a.eventType.replace(/_/g, ' ')}</Text>
              </View>
              <Text style={[styles.feedTime, { color: T.muted }]}>{timeAgo(a.occurredAt)}</Text>
            </View>
          ))}
        </View>
      )}
