...
export default function OrganizationWorkspacePage() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      const [profileData, teachersData, coursesData, usersData] = await Promise.all([
        fetchMyOrganizationProfile(),
        fetchOrganizationTeachers(),
        fetchOrganizationCourses(),
        fetchOrganizationUsers(),
    ...
    <main className={`min-h-screen bg-[#f7f9fb] px-4 py-8 ${isArabic ? "lang-ar" : "lang-en"}`}>
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">{t.organization.badge}</p>
          <h1 className="mt-2 text-3xl font-black text-slate-900">{t.organization.title}</h1>
          <p className="mt-1 text-slate-600">{t.organization.subtitle}</p>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <button
              ...
              className="ml-auto rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700"
            >
              {t.dashboard.logout}
            </button>
          </div>
                ...
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {filteredTeachers.length === 0 ? (
                      <tr>
                        <td colSpan="4" className="py-4 text-slate-500">{t.organization.common.empty}</td>
                      </tr>
