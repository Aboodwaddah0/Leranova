...
export default function Quiz({ quiz, onSubmit, submitting, isArabic }) {
  ...
  if (phase === 'in-progress') {
    ...
    const selectedOi = answers[current];
    ...
    return (
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            {questions.map((_, i) => (
              <button key={i} type="button" onClick={() => setCurrent(i)}
                className={`h-2.5 rounded-full transition-all duration-200 ${
                  ...
                  answers[i] !== undefined ? 'w-2.5 bg-indigo-300' :
                  'w-2.5 bg-slate-200'
                }`} />
            ))}
          </div>
        ...
        <div className="px-5 py-5">
          <p className="mb-5 text-base font-bold leading-relaxed text-slate-900" dir="auto">
            {q.question}
          </p>
          ...
          <div className="space-y-2.5">
            {(q.options || []).map((opt, oi) => (
              <OptionBtn key={oi} letter={String.fromCharCode(65 + oi)} text={opt}
                state={selectedOi === oi ? 'selected' : 'idle'}
  ...
  const correctCount = (reviewQs || []).filter((q, i) => attempt?.answers?.[i] === q.correctAnswer).length;
