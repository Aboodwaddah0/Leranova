...
import { CornerUpLeft, EllipsisVertical, MessageCircle, Paperclip, Pencil, SendHorizontal, Smile, Trash2, X, FileText, Download } from 'lucide-react';
...
import {
  ...
  editTeacherChatMessage,
  fetchTeacherChats,
  fetchTeacherChatMessages,
  reactTeacherChatMessage,
...
const getChatLabel = (chat, isArabic) => {
  ...
  if (type === 'SUBJECT') {
    return isArabic ? 'دردشة المادة' : 'Subject Chat';
  }
...
export default function InstructorChatPage() {
  ...
  const loadMessages = async (chatId) => {
    ...
    const nextMessages = await fetchTeacherChatMessages(chatId);
  ...
  useEffect(() => {
    ...
    const loadChats = async () => {
      try {
        setLoading(true);
        const list = await fetchTeacherChats();
    ...
    const refresh = async () => {
      try {
        const [nextChats, nextMessages] = await Promise.all([
          fetchTeacherChats(),
          fetchTeacherChatMessages(selectedChatId),
        ]);
  ...
  const handleReactToMessage = async (message, emoji) => {
    ...
    try {
      ...
      const result = await reactTeacherChatMessage(message.id, emoji);
  ...
  const handleDeleteMessage = async (messageId) => {
    ...
    try {
      ...
      await deleteTeacherChatMessage(selectedChatId, messageId);

      const refreshed = toSafeArray(await fetchTeacherChatMessages(selectedChatId));
  ...
  const handleSaveEdit = async (messageId) => {
    ...
    try {
      setSavingEdit(true);
      const updated = await editTeacherChatMessage(messageId, nextContent);
  ...
  const submitMessage = async () => {
    ...
    try {
      ...
      const message = await sendTeacherChatMessage(selectedChatId, content, replyToMessageId, filesToSend);
  ...
  return (
    <InstructorLayout
      ...
      subtitle={isArabic ? 'تواصل مع طلابك في كل مادة' : 'Communicate with your students per subject'}
    >
      {error ? (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-1.5">
                      <p className={`truncate text-sm ${chat.unreadCount ? 'font-black' : 'font-bold'}`}>{getChatLabel(chat, isArabic)}</p>
                      {chat.className ? (
                        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${active ? 'bg-white/20 text-white' : 'bg-indigo-100 text-indigo-700'}`}>
                          {chat.className}
                        </span>
                      ) : null}
                    </div>
                    <span className={`shrink-0 text-[11px] ${active ? 'text-indigo-100' : isDark ? 'text-white/35' : 'text-slate-500'}`}>
                  ...
                  <p className={`mt-1 text-[11px] ${active ? 'text-indigo-100' : isDark ? 'text-white/35' : 'text-slate-500'}`}>
                    {String(chat.type || '').toUpperCase() === 'CLASS'
                      ...
                      : (isArabic ? 'دردشة المادة' : 'Subject chat')}
                  </p>
                  {chat.unreadCount ? (
