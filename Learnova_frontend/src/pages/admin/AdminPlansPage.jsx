...
export default function AdminPlansPage() {
  ...
  useEffect(() => {
    const loadFeatures = async () => {
      try {
        ...
        notifyError(loadError.message || t.admin.plans.loadFeaturesFailed);
      }
    };
  ...
  const handleCreateFeature = async (event) => {
    ...
    try {
      ...
      await refreshAll();
      notifySuccess(t.admin.plans.featureCreatedSuccess);
    } catch (createError) {
      notifyError(createError.message || t.admin.plans.createFeatureFailed);
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleAssignFeature = async (event) => {
    ...
    try {
      ...
      notifySuccess(t.admin.plans.featureAssignedSuccess);
    } catch (assignError) {
      notifyError(assignError.message || t.admin.plans.assignFeatureFailed);
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleUpdateLimit = async (planId, featureId, nextLimit) => {
    ...
    try {
      ...
      await dispatch(fetchPlansThunk());
      notifyInfo(t.admin.plans.featureLimitUpdatedSuccess);
    } catch (updateError) {
      notifyError(updateError.message || t.admin.plans.updateLimitFailed);
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleRemoveFeature = async (planId, featureId) => {
    ...
    try {
      ...
      await dispatch(fetchPlansThunk());
      notifySuccess(t.admin.plans.featureRemovedSuccess);
    } catch (removeError) {
      notifyError(removeError.message || t.admin.plans.removeFeatureFailed);
    } finally {
      setBusy(false);
    }
  };
